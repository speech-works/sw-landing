      ...
      <div className="absolute -right-2 md:-right-4 -bottom-4 w-[140px] md:w-[170px] p-4 rounded-2xl bg-white/40 backdrop-blur-md border border-white/20 shadow-[0_15px_30px_rgba(0,0,0,0.06)] opacity-0 translate-y-10 group-[.active-stage]:opacity-100 group-[.active-stage]:translate-y-0 transition-all duration-1000 delay-300 antialiased overflow-visible">
        <div className="relative w-full aspect-square flex items-center justify-center">
          ...
          {[
            ...
            const tx =
              50 + slice.r * 0.9 * Math.cos((midAngle * Math.PI) / 180);
            const ty =
              50 + slice.r * 0.9 * Math.sin((midAngle * Math.PI) / 180);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-16 reveal text-center md:text-left active">
          <div className="max-w-2xl mx-auto md:mx-0">
            <h2 className="text-brand font-bold tracking-[0.2em] uppercase text-[10px] md:text-xs mb-3 md:mb-4">
              Professional Speech Academy
            </h2>
            <h3 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter leading-none text-app-text">
              POWERED BY
              <br />
              ADVERSARIAL AI.
            </h3>
          </div>
          <p className="text-app-muted font-medium mt-4 md:mt-0 max-w-xs mx-auto md:mx-0 md:text-right text-sm md:text-base">
            ...
            {features.map((feature, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 transition-all duration-500 rounded-full overflow-hidden ${isActive ? "h-[60%] bg-gray-200" : "h-0 bg-transparent"}`}
                  >
                    {isActive && (
                      <div
                  ...
                  <h4
                    className={`text-xl md:text-2xl font-bold tracking-tight transition-colors duration-300 ${isActive ? "text-app-text" : "text-app-muted/60 hover:text-app-text/80"}`}
                  >
                    {feature.title}
                  </h4>

                  <div
                    className={`overflow-hidden transition-all duration-500 ease-in-out ${isActive ? "max-h-48 opacity-100 mt-3" : "max-h-0 opacity-0 mt-0"}`}
                  >
