...
> = {
  left: {
    ...
    description:
      "Therapist-designed walkthroughs turn each technique from theory into something you can actually use on day one.",
    tag: "Expert-crafted",
  },
  center: {
    tab: "Dashboard",
    ...
    description:
      "Your progress, next session, and assessment score live in one screen. No searching for where you left off.",
    tag: "Progress-led",
  },
  right: {
    tab: "AI Coach",
    ...
    description:
      "The AI will rush you. Misinterpret your pause. Push back. That's the point. Hard practice makes real conversations easier.",
...
export default function Hero() {
  ...
  const getPhoneStyle = (phoneId: HeroPhoneId) => {
    ...
    const transform = `translate3d(${slotStyle.translateX}px, ${slotStyle.translateY
      }px, 0) rotate(${slotStyle.rotate}deg) scale(${slotStyle.scale + hoverScaleBoost
      })`;
          ...
          <div className="grid min-w-0 lg:grid-cols-12 gap-8 md:gap-12 items-center">
            ...
            <div className="lg:col-span-8 reveal active relative min-w-0">
              <div className="inline-flex max-w-full items-center gap-2 md:gap-3 px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl text-brand-100 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] mb-6 md:mb-8">
                ...
                For the 167 Hours Between Sessions
              </div>
              ...
              <p className="w-full max-w-none text-[1.02rem] sm:max-w-[24ch] sm:text-xl md:text-2xl text-white/70 leading-7 sm:leading-relaxed lg:max-w-2xl font-light mb-8 md:mb-10 reveal reveal-delay-1 active">
                There are people who rehearse ordering coffee before walking into a café. People who avoid saying their own name. People who know exactly what they want to say, but not whether the words will come out easily. You will find them on Speechworks.
              </p>
